...
with tab1:
    ...
    st.markdown("""<div class="grid-row" style="background-color:#222;"><div class="grid-cell" style="width:15%;">K</div><div class="grid-cell" style="width:45%;">TEAM / CAT</div><div class="grid-cell" style="width:20%;">MED</div><div class="grid-cell" style="width:20%; border:none;">ULT</div></div>""", unsafe_allow_html=True)
    ...
    for i, r in df_pista.iterrows():
        ...
        with cols[0]: st.markdown(f'<div class="grid-row"><div class="grid-cell big-num" style="width:100%; border:none;">{star}{r["KART"]}</div></div>', unsafe_allow_html=True)
        with cols[1]:
            if st.button(f"● {r['TEAM']} ({r['CAT']})", key=f"btn_pista_{i}"):
                st.session_state.sel_idx = i; st.rerun()
        with cols[2]: st.markdown(f'<div class="grid-row"><div class="grid-cell big-num" style="width:100%; color:#00FF7F; border:none;">{r["MEDIA"]:.2f}</div></div>', unsafe_allow_html=True)
        with cols[3]: st.markdown(f'<div class="grid-row"><div class="grid-cell big-num" style="width:100%; color:#666; border:none;">{r["ULTIMO"]:.1f}</div></div>', unsafe_allow_html=True)
    ...
    if n_attive > 0:
        ...
        for i, c_name in enumerate(st.session_state.corsie_attive):
            with cols_box[i]:
                ...
                st.markdown(f"<h2 style='color:{c_color}; text-align:center; border-bottom: 3px solid {c_color}; padding-bottom:10px;'>{c_name}</h2>", unsafe_allow_html=True)
                
                q = st.session_state.data[(st.session_state.data['IN_PIT'] == True) & (st.session_state.data['LANE'] == c_name)]
                for idx_b, r_b in q.iterrows():
                    m, s = divmod(int(max(0, st.session_state.tempo_pit - (time.time() - r_b['PIT_START']))), 60)
                    st.markdown(f'<div class="{css_tag}">', unsafe_allow_html=True)
                    if st.button(f"K{r_b['KART']}\n⏳ {m:02d}:{s:02d}", key=f"btn_box_{idx_b}"):
                        st.session_state.data.at[idx_b, 'IN_PIT'] = False; st.rerun()
